...
## DL7 (05/01/26) Adjusting Employee Movement + Changing States
With David's help, the Employees now have their state's changed when they walk to tables of different varieties. If an employee is sent to an empty table, they will be reset to 'idle' afterwards, whereas if it is dirty or occupied, they will be set to 'busy'. As of now, they do not take the customer's order; rather, one will spawn after a few seconds. Drag and Drop has also been implemented, with the tables randomly generating an order and the correct order being delivered resulting in the console printing 'Order delivered'.
...
## DL11 (28/01/26) Discontinued
For latest updates, see [ https://github.com/SergioIsntreal/MWIv4 ] 
